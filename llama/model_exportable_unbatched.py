...
def apply_rotary_emb(
    ...
    xq_out = torch.view_as_real(xq_ * freqs_cis).view(*xk.shape[:-1], -1)
    xk_out = torch.view_as_real(xk_ * freqs_cis).view(*xk.shape[:-1], -1)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        cache_k = cache_k.index_copy(0, input_indexes, xk)
        cache_v = cache_v.index_copy(0, input_indexes, xv)

        keys = cache_k.index_select(0, cache_indexes)
        values = cache_v.index_select(0, cache_indexes)
        ...
        output = output.transpose(-3, -2).contiguous().view(seqlen, -1)
...
class Transformer(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, tokens: torch.Tensor, input_indexes: torch.Tensor, cache_indexes, 
